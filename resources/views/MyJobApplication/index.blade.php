        ...
        <x-job-card :job="$application->job">
            <div class="flex items-center justify-between text-sm text-slate-500">
                <div>
                    ...
                    <div>Other {{Str::plural('applicant', $application->job->job_aplications_count)}} {{$application->job->job_aplications_count - 1}} </div>
                    <div>Your asking salary ${{number_format($application->expected_salary)}}</div>
                    <div>Average asking salary ${{number_format($application->job->job_aplications_avg_expected_salary)}}</div>
                </div>
